...
load_xma (string file)
{
  ...
  *((string *) loaded_anim->params[0]) = file.substr (0, file.find_last_of ("/")) + "/";
...
_render_xma (xma_anim *anim, BITMAP *draw_to, unsigned int step)
{
  ...
  /*** Commands ***/
  /* Initialisierung */
  if (step >= anim->start && !anim->initialised)
    {
      switch (anim->action_type)
        {
          case XMA_ANIMATION:
  ...
  /** Deinitialisierung **/
  if (step >= anim->length + anim->start + 1 && anim->initialised)
    {
      switch (anim->action_type)
        {
          case XMA_GROUP:
      ...
      if (anim->action_type == XMA_ANIMATION)
        stop_xma ();
    }
